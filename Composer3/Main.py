...
def groupHandling(groupItem, threadID):
    ...
    # get node info
    # for i in range(0, len(groupItem)):
        # print "==> location: ", groupItem[i].location.items()
        # nodeinfo.update(groupItem[i].location.items())
    print "\n==> nodeinfo: ", nodeinfo
    ...
    if schedule_flag:
        ...
        # find the sourceIP, it is a list
        # for i in range(0, len(groupItem)):
            # if groupItem[i].size == group_max_size:
                # continue
            # sourceIPList.append(groupItem[i].location.values()[0])
        # print "==> Source IPs are: ", sourceIPList
        # for i in range(0, len(sourceIPList)):
            # data format: [schedulingID, TARGET, sourceIP, targetIP]
        schedulingID = uuid.uuid4()
...
if __name__=='__main__':
    ...
    if len(globalList) != 0:
        ...
        # for j in range(0, len(groupList)):
            # print "==> Test GroupList: ", groupList[j][0].availableTime
        ...
        while 1:
            ...
            if (length > 0) & (length < Var.GROUP_SIZE):
                if length == 1:
                    if (length == 1) & (globalList[0].availableTime >= Var.STOP_FLAG):
                        # the final composition result in GL
                        print "Finished Composition..."
                        break
                        # just intermediately received one data set in GL
                else:
                    # length of global List is 2 or 3
                    total_avl = 0
                    for i in  range(0, length):
                        total_avl += globalList[i].availableTime
                    if total_avl == Var.STOP_FLAG:
                        # these are the final data sets that need to be composed
                        # start a thread to compose these left data sets
                        print "==> compose the last data sets in GL-----"
                        temp_group = []
                        with Var.lock:
                            for i in range(0, length):
                                temp_group.append(globalList.pop(0))
                            try:
                                ...
                                print "==> Exception: ", e
                    elif total_avl > Var.STOP_FLAG:
                        print "==> Over, time: ", time.time()
                        break
                    else:
                        # In algorithm 3, a timer is set for grouping new group no matter how many data sets in the GL
                        # print "Starting timer, after " + str(Var.TIMER_TIME) + " s, start to group..."
                        # timer =  threading.Timer(Var.TIMER_TIME, function=func_timer, args=[length])
                        # timer.start()
                        # if not Var.TIMER_FLAG:
                            # timer.cancel()
                        pass
                # dynamically change the window size
                Var.GROUP_SIZE -= 1
                if Var.GROUP_SIZE <= Var.MIN_GROUP_SIZE:
                    Var.GROUP_SIZE = Var.MIN_GROUP_SIZE
            elif length >= Var.GROUP_SIZE:
                with Var.lock:
                    group_temp = []
                    for i in range(0, Var.GROUP_SIZE):
                        group_temp.append(globalList.pop(0))
                    # create new thread to run group processing
                    try:
                        t = threading.Thread(target=groupHandling, args=(group_temp, random.randrange(10000, 20000, 100)))
                        t.start()
                    except Exception, e:
                        print "==> Exception: ", e
                    ...
                    # dynamically change the window size
                    Var.GROUP_SIZE += 1
                    if Var.GROUP_SIZE >= Var.MAX_GROUP_SIZE:
                        Var.GROUP_SIZE = Var.MAX_GROUP_SIZE
            else:
                '''time.sleep(1)
                print "==> length of global list: ", len(globalList)
                if len(globalList) > 0:
                    print "Available time in global list: "
                    for i in range(0, len(globalList)):

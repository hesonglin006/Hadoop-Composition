...
FILE_PORT = 27008
COMPOSER_PORT = 47006
SCHEDULING_CENTER = '192.168.245.235'
...
def composition(total_size, mode):
    # time complexity: n
    if mode == 1:
        time.sleep(math.floor(Cons.COMPOSITION_CONSTANT * total_size))
    # time complexity: nlogn
    elif mode == 2:
        time.sleep(math.floor(Cons.COMPOSITION_CONSTANT * total_size * math.log(total_size, 2)))
    # time complexity: n^2
    elif mode == 3:
        time.sleep(math.floor(Cons.COMPOSITION_CONSTANT * total_size * total_size))
    else:
...
if __name__=='__main__':
    ...
    try:
        schedulerThread = schedulerThread(5, "SAME_NODE_SCHEDULE_THREAD")

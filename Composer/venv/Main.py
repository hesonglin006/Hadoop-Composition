...
class Var():
    ...
    endSchedulingTime = 0
    BANDWIDTH = [([0] * 4) for i in range(4)]
...
class Dataset():
    ...
    fileinfo = ""
#
# Thread for listening on scheduler
'''
class ListeningSchedulerThread(threading.Thread):
...
def fromScheduler():
    try:
        ...
        udpSerSock.bind(('', BROADCAST_PORT))
        while True:
            data, addr = udpSerSock.recvfrom(BUFSIZE)
            print ('...From scheduler received -> %s %s' % (addr, data))
            # do something
        udpSerSock.close()
    except Exception, e:
        print "Exception: ", e
    ...
    with open('bandwidth.txt', 'r') as fd:
        ...
        for i in range(0, int(nodes[0])):
            for j in range(0, int(nodes[0])):
                Var.BANDWIDTH[i][j] = int(nodes[i + 1].split(" ")[j])
        ...
        while len(globalList) > 0:
            if Var.SCHEDULING_FLAG:
                ...
                if operatorOne.location == operatorTwo.location:
                    ...
                    schedulingIDS = str(time.time()).split(".")[0][-3:]
                    ...
                    schedulingID = str(time.time()).split(".")[0][-3:]
